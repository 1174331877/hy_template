...
namespace LT_GL
{
    public sealed class TimerMgr : AbsMgr
    {
            ...
            float repeatInterva = 1f, Action finishCb = null)
        {
            ...
            var coroutine =
                LTGL.Ins.CoroutineMgr.StartCoroutine(TimeCoroutine(timerId, callBack, delay, doTimes, repeatInterva, finishCb),
                    cancellationToken);
        ...
        public void EndTimer(long timerId)
        {
            ...
            {
                LTGL.Ins.CoroutineMgr.StopCoroutine(coroutine);
